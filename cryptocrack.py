...
__github__  = 'https://github.com/princy0708/cryptocrack'
...
from src.cipher_identifier import CipherIdentifier
        ...
        if len(encoded_base) > 3:
            ...
            if not results and not self.api_call:
                if not self.image_mode_call:
                    identified_ciphers = CipherIdentifier(encoded_base).identify()
                    if identified_ciphers:
                        print(
                            colored('\n[-] Base decoding failed, but identified as: ', 'yellow') +
                            colored(', '.join(identified_ciphers), 'cyan')
                        )
                    else:
                        push_error('Not a valid encoding and could not identify any cipher/hash.')
    ...
    def decode(self, encoded_base):
        ...
        with the structure:
            ...
            For example:
                ...
                >> CryptoCrack().decode('c3BhZ2hldHRp')
                ('spaghetti', 'Base64')
            ie:
...
def banner():
    banner = '''
   ____ ______   ______  __________  __________  ___   ________ __
  / ___/ _ \ \ / / __ \/_  __/ __ \/ ___/ __ \/ _ | / ___/ //_/
 / /__/ , _/\ V / /_/ / / / / /_/ / /__/ , _/ __ |/ /__/ ,<   
 \___/_/|_|  |_/ .___/ /_/  \____/\___/_/|_/_/ |_|\___/_/|_|  
              /_/                                             v4.0
    '''
    print(colored(banner, 'red')+colored('\n\t\tpython cryptocrack.py -h [FOR HELP]\n', 'green'))
...
def main():
    ...
    if args.file:
        if args.magic:
            ...
            CryptoCrack(output=args.output).decode_from_file(str(args.file))

    elif args.base:
        print(colored('[-] Encoded Base: ', 'yellow')+colored(str(args.base), 'red'))

        if args.magic:
            CryptoCrack().magic_mode(str(args.base))
        else:
            CryptoCrack().decode_base(str(args.base))
        ...
        if args.magic:
            CryptoCrack().magic_mode(encoded_base)
        else:
            CryptoCrack().decode_base(encoded_base)
