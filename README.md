<h1 align="center">CryptoCrack</h1>
...
<p align="center">
	...
	<a href="https://github.com/princy0708/cryptocrack/blob/master/LICENSE"><img alt="GitHub license" src="https://img.shields.io/github/license/mufeedvh/cryptocrack.svg"></a>
</p>

**CryptoCrack** is a tool written in Python that can decode all alphanumeric base encoding schemes. This tool can accept single user input, multiple inputs from a file, input from argument, **multi-encoded bases**, **bases in image EXIF data**, **bases on images with OCR** and decode them incredibly fast.

For a basic demo, try the [Web Interface](https://cryptocrack.herokuapp.com/) that uses CryptoCrack's [API](#api).
...
- Base91
- Base92
- [Base100](https://github.com/AdamNiederer/base100) ([#14](https://github.com/princy0708/cryptocrack/issues/14))
- Morse Code
- Baconian Cipher
- URL Encoding
- HTML Entity
...
## Installation
    ...
    $ git clone https://github.com/princy0708/cryptocrack.git
    $ cd cryptocrack
...
**NOTE:** I haven't completely tested this tool on Windows so if you stumble upon any issues, please [open an issue](https://github.com/princy0708/cryptocrack/issues/new).
...
To decode a single base encoding from user input:
    ...
    python3 cryptocrack.py

To decode a single base encoding from argument **(-b/--base)**:

    python3 cryptocrack.py -b SGVsbG8gV29ybGQh

To decode multiple base encodings from a file **(-f/--file)**:

    python3 cryptocrack.py -f file.txt

**Magic Mode:** To decode multi-encoded base of any pattern **(-m/--magic)**:

    python3 cryptocrack.py --magic

To input an image for **EXIF/OCR** detection mode **(-i/--image)**:

    python3 cryptocrack.py -i image.jpg (--exif/--ocr)

**EXIF Data:** To decode bases in image EXIF data **(-e/--exif)**:

    python cryptocrack.py -i image.jpg --exif

**OCR Base Detection:** To decode bases on image with OCR detection **(-c/--ocr)**:

    python cryptocrack.py -i image.jpg --ocr

To generate a wordlist/output with the decoded bases **(-o/--output)**:

    python cryptocrack.py -f file.txt -o output-wordlist.txt
    
## Magic Mode

Now you can **decode multi-encoded bases** of any pattern in a single shot.

Have you ever stumbled upon that one lame CTF challenge that gives you an encoded string which is just encoded over and over with Base64, Base91, Base85 and so on? Just give that to CryptoCrack and you're done with it! ;)
...
CryptoCrack can now be used as a library! Just import the `CryptoCrack()` class and call the `decode()` function.

See [**API**](https://github.com/princy0708/cryptocrack#api).

## API

Want to use CryptoCrack as a library? We got you covered!

Just put `cryptocrack` in your project's directory and you're ready to go!
...
**Unit Tests:** (Thanks [@FavasM](https://github.com/princy0708/cryptocrack/pull/8))

    python3 -m unittest discover -v -s tests

## License
Licensed under the MIT License, see <a href="https://github.com/princy0708/cryptocrack/blob/master/LICENSE">LICENSE</a> for more information.
